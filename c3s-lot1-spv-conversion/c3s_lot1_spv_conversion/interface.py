...
def compute_spv(
    ...
    out_filename: Optional[str] = "default",
    ...
    dt_downscale: int, optional
        Time resolution to which data is downscaled, in minutes, to better
            account for variation in angle of incidence. Default: 15,
            but by the end of the code the data is reaggregated to hourly.
    pv_params: dict, optional
        Customizable PV parameters. Default: None.
    print_progress : bool
        Print computed pixels to keep track of progress. Default: False.
    ...
    out_dir: Optional[str]
        Directory where to store output .nc file. Default: None.
    ...
    # global variables
    global n_p, dt_or, dt_ds, print_prog
    global pv_tilt, pv_azim, th_coef, meta
    global SSRD, T2M, meta
    ...
    pv_tilt, pv_azim, excl_mask = read_support_inputs(
        meta, SSRD.shape, in_excl_mask_path, pv_params
    )
    ...
    # test_for_nans(SSRD, "ssrd")
    # test_for_nans(T2M, "t2m")
    ...
    if to_save:
        msg = "out_dir variable must be str"
        assert isinstance(out_dir, str), msg

        msg = "out_filename variable must be str"
        assert isinstance(out_filename, str), msg

        # default assumes same name as ssrd file, replacing GHI by SPV
        if out_filename == "default":
            temp = in_ssrd_path.split()[-1]
            out_filename = temp.replace("GHI", "SPV")

        # creates and encodes output .nc file, stores photovoltaic
        # capacity factor
        prep_output_file(
            path.join(out_dir, out_filename),
            DataStream,
            out_all,
            meta["vLon"],
            meta["vLat"],

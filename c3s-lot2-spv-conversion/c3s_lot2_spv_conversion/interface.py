...
from c3s_lot2_spv_conversion.utils.support_functions import (
    prep_output_file,
)
from c3s_lot2_spv_conversion.utils.unit_tests import (
...
def compute_spv(
    ...
    DataStream: str
        IDs data as historical (HIST) or projection (PROJ).
    ...
    # check if provided pv_typology is valid
    test_pv_typology(pv_typology)
    ...
    if in_excl_mask_path == "default":
        ...
        with pkg_resources.path(anci, "default_mask_name.json") as template:
            with open(template, "r") as f:
                mask_file_name = json.load(f)["excl_mask_path"]
